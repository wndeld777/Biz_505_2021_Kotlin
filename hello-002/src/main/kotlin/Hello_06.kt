...
fun main() {
    ...
    // 다시 입력받고자 하는 대상이 정수형인데 문자열로 입력받았으므로
    // 다시 정수형으로 변환해 주어야 한다
    // var intNum = Integer.valueOf(strNum)

    // 정수가 아닌 값을 입력하면 Integer.valueOf() 에서 exception 이 발생한다
    // 이때 try-catch 를 사용하여 exception 처리를 해주어야 한다

    var intNum = 0
    try{
        intNum = Integer.valueOf(strNum)
    }catch (e:NumberFormatException){
        println("정수만 입력해야 합니다 변환 불가")
    }
